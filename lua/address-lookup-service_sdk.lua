...
require("address-lookup-service_types")
...
local AddressLookupServiceSDK = {}
AddressLookupServiceSDK.__index = AddressLookupServiceSDK
...
AddressLookupServiceSDK._make_feature = _make_feature


function AddressLookupServiceSDK.new(options)
  local self = setmetatable({}, AddressLookupServiceSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function AddressLookupServiceSDK:options_map()
...
function AddressLookupServiceSDK:get_utility()
  return Utility.copy(self._utility)
end


function AddressLookupServiceSDK:get_root_ctx()
  return self._rootctx
end


function AddressLookupServiceSDK:prepare(fetchargs)
...
function AddressLookupServiceSDK:direct(fetchargs)
...
function AddressLookupServiceSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function AddressLookupServiceSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "AddressLookupServiceSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function AddressLookupServiceSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function AddressLookupServiceSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "AddressLookupServiceSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function AddressLookupServiceSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = AddressLookupServiceSDK.new(sdkopts)
  sdk.mode = "test"
...
return AddressLookupServiceSDK
