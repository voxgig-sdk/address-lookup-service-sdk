# address-lookup-service-cli

boru-driven command-line client **and** interactive REPL for the AddressLookupService
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/address-lookup-service-cli)
make build

# 2. See usage (words, entities, env vars)
./address-lookup-service-cli --help

# 3. Provide credentials once, via the environment
export ADDRESS_LOOKUP_SERVICE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./address-lookup-service-cli list search_addresses_get
./address-lookup-service-cli list search_addresses_post

# 5. Override the API base URL for a single call
ADDRESS_LOOKUP_SERVICE_BASE=https://api.example.com ./address-lookup-service-cli list search_addresses_get

# 6. No arguments -> interactive REPL
./address-lookup-service-cli
address-lookup-service> list search_addresses_get
address-lookup-service> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/address-lookup-service-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/address-lookup-service-cli list search_addresses_get
   ```
...
Run with no arguments to open a REPL (prompt `address-lookup-service>`). Each line is
evaluated as its own boru expression:

```text
$ ./address-lookup-service-cli
address-lookup-service> list search_addresses_get
address-lookup-service> /help
address-lookup-service> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `search_addresses_get`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/address-lookup-service-cli`. |
...
(`./address-lookup-service-cli <expr>`) and interactively (the REPL), and expressions compose the
