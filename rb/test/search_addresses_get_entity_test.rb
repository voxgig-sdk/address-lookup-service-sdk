...
class SearchAddressesGetEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = AddressLookupServiceSDK.test(seed, nil)
    seen = base.SearchAddressesGet(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.SearchAddressesGet(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ADDRESS_LOOKUP_SERVICE_TEST_SEARCH_ADDRESSES_GET_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def search_addresses_get_basic_setup(extra)
  ...
  entid_env_raw = ENV["ADDRESS_LOOKUP_SERVICE_TEST_SEARCH_ADDRESSES_GET_ENTID"]
