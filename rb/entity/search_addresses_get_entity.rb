...
class SearchAddressesGetEntity
  ...
  def make
    opts = @_entopts.dup
    SearchAddressesGetEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AddressLookupServiceHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [SearchAddressesGet, Hash] the current SearchAddressesGet data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = AddressLookupServiceHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of SearchAddressesGet fields)
  def match_get
  ...
  #
  # @param reqmatch [SearchAddressesGetListMatch, Hash, nil] match filter (any subset of
  #   SearchAddressesGet fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<SearchAddressesGet>, Array] the matching SearchAddressesGet items; raises AddressLookupServiceError on failure
