...
require_relative 'AddressLookupService_types'
...
class AddressLookupServiceSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = AddressLookupServiceHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = AddressLookupServiceHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, AddressLookupServiceFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    AddressLookupServiceUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = AddressLookupServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = AddressLookupServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = AddressLookupServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => AddressLookupServiceError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue AddressLookupServiceError => err
    ...
    if fetched.is_a?(Hash)
      status = AddressLookupServiceHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = AddressLookupServiceError.new(
        "graphql_error", "AddressLookupServiceSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = AddressLookupServiceSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
