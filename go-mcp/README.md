...
MCP server exposing the AddressLookupService SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `address-lookup-service_list` | `entity`, optional `query` map | First page of records as JSON |
| `address-lookup-service_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
```sh
go build -o address-lookup-service-mcp ./...
```
...
After install, restart Claude Code; the `address-lookup-service_list` and
`address-lookup-service_load` tools become available in new sessions.
