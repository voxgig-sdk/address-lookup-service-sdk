...
type Args struct {
	Entity string         `json:"entity" jsonschema:"search_addresses_get | search_addresses_post"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.AddressLookupServiceSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "address-lookup-service_list",
		Description: "List records from AddressLookupService. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "address-lookup-service_load",
		Description: "Load a single record from AddressLookupService. " +
...
func runOp(client *sdk.AddressLookupServiceSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.AddressLookupServiceSDK, name string) (sdk.AddressLookupServiceEntity, error) {
	switch strings.ToLower(name) {
	case "search_addresses_get":
		return client.SearchAddressesGet(nil), nil
	case "search_addresses_post":
		return client.SearchAddressesPost(nil), nil
