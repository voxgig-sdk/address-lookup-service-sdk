...
class TestSearchAddressesGetDirect:

    def test_should_direct_list_search_addresses_get(self):
        setup = _search_addresses_get_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-search_addresses_get", "live" if setup["live"] else "unit")
        if _skip:
...
def _search_addresses_get_direct_setup(mockres):
