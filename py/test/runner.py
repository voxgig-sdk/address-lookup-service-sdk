# AddressLookupService SDK test runner
...
class AddressLookupServiceTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AddressLookupServiceTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AddressLookupServiceTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AddressLookupServiceTestRunner.getenv("ADDRESS_LOOKUP_SERVICE_TEST_LIVE")
        override = AddressLookupServiceTestRunner.getenv("ADDRESS_LOOKUP_SERVICE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AddressLookupServiceTestRunner.getenv(key)
        ...
        explain = AddressLookupServiceTestRunner.getenv("ADDRESS_LOOKUP_SERVICE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ADDRESS_LOOKUP_SERVICE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if AddressLookupServiceTestRunner._test_control is not None:
            return AddressLookupServiceTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AddressLookupServiceTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AddressLookupServiceTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AddressLookupServiceTestRunner.load_test_control()
...
def load_env_local():
    AddressLookupServiceTestRunner.load_env_local()


def env_override(m):
    return AddressLookupServiceTestRunner.env_override(m)


def entity_data(v):
    return AddressLookupServiceTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AddressLookupServiceTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AddressLookupServiceTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AddressLookupServiceTestRunner.load_test_control()


def live_client_options():
    return AddressLookupServiceTestRunner.live_client_options()


def live_delay_ms():
    return AddressLookupServiceTestRunner.live_delay_ms()
