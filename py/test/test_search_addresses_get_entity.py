...
class TestSearchAddressesGetEntity:

    def test_should_create_instance(self):
        testsdk = AddressLookupServiceSDK.test(None, None)
        ent = testsdk.SearchAddressesGet(None)
    ...
    def test_should_stream(self):
        ...
        base = AddressLookupServiceSDK.test(seed, None)
        seen = list(base.SearchAddressesGet(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = AddressLookupServiceSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.SearchAddressesGet(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _search_addresses_get_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "search_addresses_get." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ADDRESS_LOOKUP_SERVICE_TEST_SEARCH_ADDRESSES_GET_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        search_addresses_get_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.search_addresses_get")))
        search_addresses_get_ref01_data = None
        if len(search_addresses_get_ref01_data_raw) > 0:
...
def _search_addresses_get_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/search_addresses_get/SearchAddressesGetTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("ADDRESS_LOOKUP_SERVICE_TEST_SEARCH_ADDRESSES_GET_ENTID"))
