...
// AddressLookupService SDK feature test
//
...
class FtCtrl extends AddressLookupServiceControl
{
...
class FtClient
{
    ...
    private AddressLookupServiceUtility $utility;

    public function __construct(AddressLookupServiceUtility $utility, array $options)
    {
    ...
    public function get_utility(): AddressLookupServiceUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AddressLookupServiceContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AddressLookupServiceContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AddressLookupServiceContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AddressLookupServiceConfig::make_feature($name);
    ...
    private static function buildUrl(AddressLookupServiceSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AddressLookupServiceError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AddressLookupServiceResponse($fetched);
        }
    ...
    private function fail(AddressLookupServiceContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AddressLookupServiceContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
