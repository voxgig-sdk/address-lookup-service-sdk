...
// AddressLookupService SDK retry feature
...
class AddressLookupServiceRetryFeature extends AddressLookupServiceBaseFeature
{
    ...
    public function init(AddressLookupServiceContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AddressLookupServiceContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AddressLookupServiceContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AddressLookupServiceContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
