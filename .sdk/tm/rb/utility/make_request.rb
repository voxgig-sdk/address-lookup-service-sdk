...
module AddressLookupServiceUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AddressLookupServiceResponse.new({})
    ...
    if fetch_err
      ...
      response = AddressLookupServiceResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AddressLookupServiceResponse.new(fetched)
    else
