...
//     (\`new AddressLookupServiceSDK(...)\` / \`AddressLookupServiceSDK.test(...)\` become
//     \`AddressLookupServiceSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'AddressLookupServiceSDK'
...
const TEST_SEED = {"entity":{"search_addresses_get":{"test01":{"id":"test01"}},"search_addresses_post":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AddressLookupServiceSDK(...)\` and any
// \`AddressLookupServiceSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AddressLookupServiceSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AddressLookupServiceSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
