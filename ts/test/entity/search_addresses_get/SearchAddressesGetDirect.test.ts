...
describe('SearchAddressesGetDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new AddressLookupServiceSDK({
  ...
  test('direct-list-search_addresses_get', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-search_addresses_get', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ADDRESS_LOOKUP_SERVICE_TEST_SEARCH_ADDRESSES_GET_ENTID']
