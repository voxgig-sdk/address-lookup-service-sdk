...
// AddressLookupService SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class AddressLookupServiceTestFeature extends AddressLookupServiceBaseFeature
{
    ...
    public function init(AddressLookupServiceContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (AddressLookupServiceContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = AddressLookupServiceParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (AddressLookupServiceContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(AddressLookupServiceContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = AddressLookupServiceParam::call($ctx, $k);
