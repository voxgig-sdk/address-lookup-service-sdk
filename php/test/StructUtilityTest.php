...
// AddressLookupService SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = AddressLookupServiceOmni::makeStructRunner(
    ...
    {
        return AddressLookupServiceOmni::objmodel(AddressLookupServiceOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return AddressLookupServiceOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return AddressLookupServiceOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(AddressLookupServiceOmni::structfix($now, Struct::undef()))
                        !== json_encode(AddressLookupServiceOmni::structfix($val, Struct::undef()))) {
