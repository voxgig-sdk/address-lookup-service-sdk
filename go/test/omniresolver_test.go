...
import (
	...
	omni "github.com/voxgig-sdk/address-lookup-service-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AddressLookupServiceError); is {
				out := map[string]any{
					"name":    "AddressLookupServiceError",
...
func omniCtx(arg any, client *sdk.AddressLookupServiceSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AddressLookupServiceSDK); is {
			client = live
