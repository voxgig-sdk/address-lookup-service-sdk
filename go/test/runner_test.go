...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("ADDRESSLOOKUPSERVICE_TEST_LIVE") == "TRUE" ||
		os.Getenv("ADDRESSLOOKUPSERVICE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("ADDRESSLOOKUPSERVICE_TEST_EXPLAIN"); explain != "" {
